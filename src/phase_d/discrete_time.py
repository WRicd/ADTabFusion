            ...
            row.update(
                {
                    ...
                    "interval_start_months": previous,
                    "elapsed_months": end,
                    "event": int(event),
                    "label": int(event),
                }
...
def train_discrete_time_model(config: dict[str, Any]) -> dict[str, Any]:
    ...
    train, validation, test = (
        periods[periods["split"] == name].copy() for name in ("train", "validation", "temporal_test")
    )
    rows, candidates = [], {}
    for model_name in config["models"]["run"]:
        ...
        metrics = binary_metrics(
            validation["label"].to_numpy(), pipeline.predict_proba(validation[model_features])[:, 1]
        )
    ...
    pd.DataFrame([{"model": name, "split": "locked_temporal_test", "n_rows": len(test), **test_metrics}]).to_csv(
        output / "temporal_validation" / "mci_discrete_time_metrics.csv", index=False
    )
    results.to_csv(output / "internal_validation" / "mci_discrete_time_validation.csv", index=False)
    model_path = output / "models" / "mci_discrete_time_pipeline.joblib"
    joblib.dump(pipeline, model_path)
    manifest = {
        "model_id": f"phase_d_mci_discrete_{name}",
        "model_name": name,
        "feature_order": model_features,
        "interval_ends_months": config["intervals"],
...
def predict_cumulative_risk(
    pipeline, index_visits: pd.DataFrame, features: list[str], interval_ends: list[int]
) -> pd.DataFrame:
    rows = []
    for _, subject in index_visits.iterrows():
            ...
            rows.append(
                {
                    ...
                    "horizon_months": end,
                    "hazard": hazard,
                    "cumulative_risk": 1.0 - cumulative_survival,
                }
            )
...
def _plot_risk(risk: pd.DataFrame, path: Path) -> None:
    ...
    ax.plot(summary.index, summary["median"], marker="s", label="Median risk")
    ax.set_ylim(0, 1)
    ax.set_xlabel("Horizon (months)")
    ax.set_ylabel("Cumulative AD Conversion Risk")
    ax.set_title("MCI Conversion Risk by Horizon")
    ax.legend()
    fig.tight_layout()
