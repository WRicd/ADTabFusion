        ...
        for index, source in enumerate(records[:-1]):
            for future in records[index + 1 :]:
                ...
                row = {
                    ...
                    "SOURCE_DX": source["_dx"],
                    "FUTURE_DX": future["_dx"],
                    "label": MULTICLASS_MAPPING[future["_dx"]],
...
def load_transition_data(config: dict[str, Any]) -> tuple[pd.DataFrame, list[str], dict[str, list[Any]]]:
    ...
    pairs = build_transition_pairs(
        frame,
        features,
        pair_cfg.get("min_horizon_months", 6),
        pair_cfg.get("max_horizon_months", 60),
...
def train_transition_aware(config: dict[str, Any], config_path: str | Path) -> dict[str, Any]:
    ...
    model_names = config["models"]["run"]
    total_fits = len(ABLATIONS) * len(model_names)
    completed = 0
    LOGGER.info(
        "Fitting %d model/ablation combinations (%d pairs, %d features).",
        total_fits,
        len(train),
        len(base_features),
    )
    for ablation, resolver in ABLATIONS.items():
        ...
        for model_name in model_names:
            ...
            pipeline = _fit_transition_pipeline(train, features, model_name, config, early_stopping_frame=validation)
            metrics = _evaluate(pipeline, validation, features)
            completed += 1
            LOGGER.info(
                "[%d/%d] %s / %s -- validation macro F1 %.4f (%.1fs)",
                completed,
                total_fits,
                ablation,
                model_name,
    ...
    # Tie-break toward the simpler model when validation metrics are equal.
    # Unlisted models sort last so a new architecture never wins on a tie.
    complexity = {
    ...
    full_results["complexity"] = full_results["model"].map(complexity).fillna(99).astype(int)
    ...
    full_results.drop(columns="complexity").to_csv(
        output / "internal_validation" / "transition_model_results.csv", index=False
    )
    ...
    _prediction_frame(selected_pipeline, validation, full_features).to_csv(
        output / "internal_validation" / "transition_validation_predictions.csv", index=False
    )
    _prediction_frame(selected_pipeline, temporal_test, full_features).to_csv(
        output / "temporal_validation" / "transition_test_predictions.csv", index=False
    )
...
def _fit_transition_pipeline(
    frame: pd.DataFrame,
    features: list[str],
    model_name: str,
    config: dict[str, Any],
    early_stopping_frame: pd.DataFrame | None = None,
) -> Pipeline:
    """Fit the preprocessor and estimator for one model/feature combination.

    *early_stopping_frame* is only consumed by models configured with
    ``early_stopping: true``, and only to decide when to stop -- it is never
    added to the training rows. It is deliberately omitted inside
    cross-validation, where the held-out fold must stay unseen.
    """
    ...
    X_val = y_val = None
    if early_stopping_frame is not None and not early_stopping_frame.empty:
        X_val = preprocessor.transform(early_stopping_frame[features])
        y_val = early_stopping_frame["label"].to_numpy()
    estimator = fit_model(
        model_name,
        transformed,
        frame["label"].to_numpy(),
        config["models"],
        frame["subject_weight"].to_numpy(),
        X_val=X_val,
        y_val=y_val,
    )
...
def _prediction_frame(pipeline: Pipeline, frame: pd.DataFrame, features: list[str]) -> pd.DataFrame:
    ...
    for index, name in enumerate(("CN", "MCI", "AD")):
        result[f"prob_{name}"] = probability[:, index]
    result["confidence"] = probability.max(axis=1)
    return result


def _run_group_kfold(
    frame: pd.DataFrame, features: list[str], model_name: str, config: dict[str, Any], output: Path
) -> None:
    rows = []
...
def _write_pair_statistics(pairs: pd.DataFrame, output: Path) -> None:
    ...
    summary = {
        "subjects": int(pairs["RID"].nunique()),
        "total_pairs": int(len(pairs)),
        "median_pairs_per_subject": float(counts.median()),
        "maximum_pairs_per_subject": int(counts.max()),
        "effective_weighted_sample_size": float(pairs["subject_weight"].sum()),
        "per_subject_weight_sum_min": float(weights.min()),
        "per_subject_weight_sum_max": float(weights.max()),
...
def _transition_manifest(
    ...
    return {
        ...
        "source_label_name": "SOURCE_DX",
        "target_label_name": "FUTURE_DX",
        "subject_balanced_pair_weights": True,
        "selection_metric": "validation_macro_f1",
        "training_subject_count": int(train["RID"].nunique()),
...
def _plot_ablation(frame: pd.DataFrame, path: Path) -> None:
    ...
    ax.set_title("Transition-Aware Feature Ablation")
    fig.tight_layout()
