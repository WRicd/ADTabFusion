...
def risk_coverage_rows(
    ...
    for target_coverage, threshold in thresholds.items():
        ...
        metrics = compute_metrics(
            y_true[retained], probability[retained].argmax(axis=1), probability[retained], labels=[0, 1, 2]
        )
        rows.append(
            {
                "split": split,
...
def evaluate_selective_prediction(config: dict[str, Any]) -> dict[str, Any]:
    ...
    rows = risk_coverage_rows(
        validation.label.to_numpy(), validation_probability, thresholds, "validation"
    ) + risk_coverage_rows(test.label.to_numpy(), test_probability, thresholds, "locked_temporal_test")
    result = pd.DataFrame(rows)
    minimum = config["threshold_selection"].get("minimum_coverage", 0.7)
    ...
    pd.DataFrame({"measure": measures.columns, "mean": measures.mean().values, "std": measures.std().values}).to_csv(
        output / "uncertainty" / "uncertainty_summary.csv", index=False
    )
    manifest = {
        "threshold": selected_threshold,
        "selection_split": "D1/D2 validation subjects only",
...
def _plot_risk_coverage(frame: pd.DataFrame, path: Path) -> None:
    ...
    for split, group in frame.groupby("split"):
        ...
        ax.plot(group.coverage, group.error_rate, marker="o", label=split.replace("_", " ").title())
    ax.set_xlabel("Coverage")
    ax.set_ylabel("Error Rate")
    ax.set_title("Selective Prediction Risk-Coverage")
    ax.legend()
    fig.tight_layout()
