...
def classwise_calibration_error(y_true: np.ndarray, probability: np.ndarray, bins: int = 10) -> dict[str, float]:
    ...
    for class_id, name in enumerate(("CN", "MCI", "AD")):
        ...
        for index in range(bins):
            ...
            if mask.any():
                error += mask.mean() * abs(target[mask].mean() - score[mask].mean())
...
def calibrate_phase_d(config: dict[str, Any]) -> dict[str, Any]:
    ...
    for method in config["methods"]:
        calibrated = ProbabilityCalibratedClassifier(base, method).fit_calibration(
            validation[features], validation["label"].to_numpy()
        )
        ...
        for split_name, frame in (("validation", validation), ("locked_temporal_test", temporal_test)):
            ...
            metrics = compute_metrics(
                frame["label"].to_numpy(), probability.argmax(axis=1), probability, labels=[0, 1, 2]
            )
            ...
            rows.append(
                {
                    ...
                    "ece": expected_calibration_error(
                        frame["label"].to_numpy(), probability, config.get("ece_bins", 10)
                    ),
    ...
    manifest = {
        ...
        "calibrated_model_sha256": sha256_file(model_path),
    ...
    for class_id, name in enumerate(("CN", "MCI", "AD")):
        for model, label, color in ((base, "Uncalibrated", "#4c78a8"), (calibrated, "Calibrated", "#e15759")):
            ...
            for bin_id in range(10):
                mask = bins == bin_id
                if mask.any():
                    x.append(score[mask].mean())
                    y.append((truth[mask] == class_id).mean())
            axes[class_id].plot(x, y, marker="o", label=label, color=color)
        axes[class_id].plot([0, 1], [0, 1], "--", color="gray")
        axes[class_id].set_title(name)
        axes[class_id].set_xlabel("Predicted probability")
    axes[0].set_ylabel("Observed frequency")
    axes[-1].legend()
    fig.suptitle("Temporal-Test Reliability by Class")
    fig.tight_layout()
