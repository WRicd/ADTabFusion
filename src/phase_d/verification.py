...
def verify_phase_c_artifacts(root: str | Path = "outputs/phase_c") -> dict[str, Any]:
    ...
    specs = [
        (
            "primary",
            root / "models/primary_pipeline.joblib",
            root / "manifests/primary_model_manifest.json",
            Path("configs/phase_c_direct_transfer.yaml"),
        ),
        (
            "sensitivity",
            root / "models/sensitivity_pipeline.joblib",
            root / "manifests/sensitivity_model_manifest.json",
            Path("configs/phase_c_direct_transfer.yaml"),
        ),
        (
            "horizon_aware",
            root / "models/horizon_aware_pipeline.joblib",
            root / "manifests/horizon_aware_manifest.json",
            Path("configs/phase_c_horizon_aware.yaml"),
        ),
    ]
    manifests = {}
    ...
    if direct_manifests:
        ...
        for role in direct_manifests:
            ...
            _check(
                checks,
                f"{role}_probability_simplex",
                np.allclose(stored[probability_columns].sum(axis=1), 1.0, atol=1e-8),
                "sum=1",
            )
    ...
    if horizon and hashes["horizon_aware"]["model_sha256"] == horizon["model_sha256"]:
        ...
        _check(
            checks,
            "horizon_d3_d4_reproduced",
            np.allclose(reproduced[columns], stored[columns], atol=1e-12),
            f"{len(stored)} rows",
        )
