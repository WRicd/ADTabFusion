...
def generate_transition_error_analysis(output_root: str | Path = "outputs/phase_d") -> dict[str, int]:
    ...
    groups = {
        "MCI_to_AD_missed_conversions": (
            (predictions.SOURCE_DX == "MCI") & (predictions.FUTURE_DX == "AD") & (predictions.PREDICTED_DX != "AD")
        ),
        "MCI_to_MCI_false_progression": (
            (predictions.SOURCE_DX == "MCI") & (predictions.FUTURE_DX == "MCI") & (predictions.PREDICTED_DX == "AD")
        ),
        "CN_progression_missed": (
            (predictions.SOURCE_DX == "CN")
            & (predictions.FUTURE_DX.isin(["MCI", "AD"]))
            & (predictions.PREDICTED_DX == "CN")
        ),
    ...
    lines.extend(
        [
            ...
            "### By Future Class",
            "",
            errors["FUTURE_DX"].value_counts().to_string(),
            "",
            "### By Source Diagnosis",
            "",
            errors["SOURCE_DX"].value_counts().to_string(),
            "",
            "### By Forecast Horizon",
            "",
            errors["forecast_months"].describe().to_string(),
            "",
            "### Confidence",
            "",
            errors["confidence"].describe().to_string(),
            "",
        ]
    )
