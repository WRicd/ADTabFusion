...
def test_calibration_and_selective_operating_point_match_frozen_artifacts() -> None:
    ...
    assert error is None
    isotonic = calibration.loc[
        (calibration["split"] == "locked_temporal_test") & (calibration["method"] == "isotonic")
    ].iloc[0]
    ...
    selected = selective.loc[
        (selective["split"] == "locked_temporal_test")
        & (selective["selected_validation_threshold"].astype(str).str.lower() == "true")
    ].iloc[0]
