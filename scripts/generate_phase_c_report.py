...
def _metric_line(label: str, row: pd.Series) -> str:
    values = [
        row.get(name)
        for name in ["accuracy", "balanced_accuracy", "macro_f1", "roc_auc_ovr", "log_loss", "brier_score"]
    ]
    return (
        "| " + label + " | " + " | ".join("NA" if pd.isna(value) else f"{float(value):.3f}" for value in values) + " |"
    )
...
def _generate_figures(root: Path, direct: pd.DataFrame, horizon: pd.DataFrame, matched: pd.DataFrame) -> None:
    ...
    ax.set_xticks(range(3), ["CN", "MCI", "AD"])
    ax.set_yticks(range(3), ["CN", "MCI", "AD"])
    ax.set_xlabel("Predicted Diagnosis")
    ax.set_ylabel("D4 Diagnosis")
    ax.set_title("External Direct-Transfer Confusion Matrix")
    for i in range(3):
        for j in range(3):
            ax.text(j, i, str(cm[i, j]), ha="center", va="center")
    fig.colorbar(image, ax=ax)
    fig.tight_layout()
    fig.savefig(figures / "confusion_matrix_external.png", dpi=160)
    plt.close(fig)
    ...
    ax.set_title("External Performance by Forecast Horizon")
    ax.legend()
    fig.tight_layout()
    fig.savefig(figures / "performance_by_horizon.png", dpi=160)
    plt.close(fig)
    ...
    ax.set_xticks(x, ["Accuracy", "Balanced Accuracy", "Macro F1", "ROC-AUC OvR"])
    ax.set_ylim(0, 1)
    ax.set_ylabel("Score")
    ax.set_title("Direct Transfer vs Horizon-Aware Future Diagnosis")
    ax.legend()
    fig.tight_layout()
    fig.savefig(figures / "direct_vs_horizon_aware.png", dpi=160)
    plt.close(fig)
