...
def main() -> None:
    ...
    uncalibrated_test = calibration[
        (calibration.split == "locked_temporal_test") & (calibration.method == "uncalibrated")
    ].iloc[0]
    ...
    for _, row in mci.iterrows():
        lines.append(
            f"| {int(row.horizon_months)} months | {int(row.n_subjects)} | {row.roc_auc:.3f} | {row.pr_auc:.3f} | {row.balanced_accuracy:.3f} | {row.macro_f1:.3f} |"
        )
    lines.extend(
        [
            "",
